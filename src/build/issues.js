...
export default class Issues {
    ...
    static buildRow(issue) {
        ...
        if (issue["assignees"].length !== 0) {
            ...
            if (issue["assignees"].length > 1) {
                ...
                assignee_link.title = issue["assignees"].slice(2).reduce((previous, current) => previous + ", " + current["login"], issue["assignees"][1]["login"]);
            }
     ...
     */
    static buildLinkOpenInNewTab(address, name) {
