...
export default class Pinned {
    ...
    private static add(name, append = true) {
    ...
    private static remove(to_be_removed) {
     ...
     */
    static reorder(rows: Array<HTMLDivElement>): Array<HTMLDivElement> {
        const new_rows: Array<HTMLDivElement> = [];
        const indices_to_remove: Array<number> = [];
        ...
        for (const [ind, row] of rows.entries()) {
            repository_name = (<HTMLAnchorElement>row.children[0]).text;
